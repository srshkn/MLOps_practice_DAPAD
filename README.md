...
Текущая реализация в `lab2` хранит подготовленный датасет в `lab2/dataset` и публикует его через DVC только если quality gate проходит (`accuracy >= 0.8`). Для Jenkins job нужно задать как минимум переменную окружения `DVC_REMOTE_URL`; при использовании S3-совместимого хранилища можно также задать `DVC_S3_ENDPOINT_URL`.
